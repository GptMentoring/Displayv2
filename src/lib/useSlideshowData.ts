...
const Slideshow: React.FC = () => {
  ...
  const renderQuadrantLayoutContent = () => {
    ...
    const bottomLeftIframe = settings.quadrantConfig.bottomLeft.contentId
      ? iframeItems.find(item => item.id === settings.quadrantConfig.bottomLeft.contentId)
      : iframeItems[0];

    const bottomRightIframe = settings.quadrantConfig.bottomRight.contentId
      ? iframeItems.find(item => item.id === settings.quadrantConfig.bottomRight.contentId)
      : iframeItems[1];
            ...
            {settings.layoutMode === 'quadrant' && (
              <div className="space-y-4 mt-2 p-3 bg-gray-50 rounded-md border border-gray-200">
                ...
                <div>
                  ...
                  <select
                    id="bottomLeftIframe"
                    value={settings.quadrantConfig.bottomLeft.contentId || ''}
                    onChange={(e) => updateGlobalSettings(prev => ({
                      ...prev,
                      quadrantConfig: {
                        ...prev.quadrantConfig,
                        bottomLeft: { type: 'iframe', contentId: e.target.value || null }
                      }
                    }))}
                    className="w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                ...
                <div className="mt-4">
                  ...
                  <select
                    ...
                    value={settings.quadrantConfig.bottomRight.contentId || ''}
                    onChange={(e) => updateGlobalSettings(prev => ({
                      ...prev,
                      quadrantConfig: {
                        ...prev.quadrantConfig,
                        bottomRight: { type: 'iframe', contentId: e.target.value || null }
                      }
                    }))}
                    className="w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                    disabled={iframeItems.length === 0}
