/*
  ...
  1. New Tables
    - `settings_history`
      - `id` (uuid, primary key)
      - `settings_id` (text, foreign key to settings.id)
      - `value` (text)
  ...
  2. Security
    ...
    - Add policies for:
      ...
      - No update/delete policies (history should be immutable)

  3. Foreign Keys
    - `settings_id` references `settings(id)`
*/

-- Create the settings_history table
...
-- Create an index on settings_id and created_at for efficient querying
CREATE INDEX IF NOT EXISTS settings_history_settings_id_created_at_idx 
  ON settings_history(settings_id, created_at DESC);

-- Add a trigger to automatically create history entries when settings are updated
