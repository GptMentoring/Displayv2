/*
  ...
  1. Changes
    ...
    - Fix the OLD table reference issue
    - Maintain same security rules and validation

  2. Security
    - Only authenticated users can update items
    - Separate validation rules for sort_order updates vs other updates
...
WITH CHECK (
  CASE 
    WHEN sort_order IS NOT NULL THEN
      -- For sort_order updates, verify no other fields are being modified
